...
const Navbar = () => {
  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-4 bg-blue-400 shadow-sm items-center'>
        ...
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            ...
            <li className='py-2 px-3 cursor-pointer text-blue-500 bg-gray-100 transition rounded-sm'><Link to="/login">Login</Link></li>
          )}
          
            <li className='py-2 px-3 cursor-pointer text-blue-500 hover:bg-gray-100 transition rounded-lm'><Link to="/signup">Signup</Link></li>
        </ul>
        ...
        <div className={`absolute md:hidden right-0 top-0 bottom-0 transition ${(isNavbarOpen === true) ? 'translate-x-0' : 'translate-x-full'} bg-gray-100 shadow-md w-screen sm:w-9/12 h-screen`}>
          ...
          <ul className='flex flex-col gap-4 uppercase font-medium text-center'>
            {authState.isLoggedIn ? (
              <>
                {/* <li className='py-2 px-3 cursor-pointer hover:bg-gray-200 transition rounded-sm' onClick={handleLogoutClick}>Login</li> */}
